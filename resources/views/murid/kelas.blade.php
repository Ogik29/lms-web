...
<div class="container-fluid">
    ...
    <h1 class="h3 mb-4">Daftar Kelas Anda</h1>
            <div class="card card-modern shadow-modern mb-4 animate-fade-in">
                <div class="card-body">
                    <h5 class="card-title fw-bold"><i class="fas fa-user-plus me-2 text-primary"></i>Gabung Kelas Baru</h5>
                    <p class="card-text text-muted">Masukkan 8 digit kode kelas yang diberikan oleh guru Anda untuk bergabung.</p>
                    <form action="{{ route('student.courses.join') }}" method="POST" class="row g-2 align-items-center">
                        @csrf
                        <div class="col-sm-8">
                            <label for="code" class="visually-hidden">Kode Kelas</label>
                            <input type="text" class="form-control form-control-modern" name="code" id="code"
                                   placeholder="CONTOH: A1B2C3D4" maxlength="8" style="text-transform:uppercase" required>
                        </div>
                        <div class="col-sm-4">
                            <button type="submit" class="btn btn-modern-primary w-100">Gabung</button>
                        </div>
                    </form>
                    <!-- Tampilan Pesan Error/Sukses -->
                    @if (session('success'))
                        <div class="alert alert-modern alert-modern-success mt-3 mb-0">{{ session('success') }}</div>
                    @endif
                    @if (session('error'))
                        <div class="alert alert-modern alert-modern-danger mt-3 mb-0">{{ session('error') }}</div>
                    @endif
                    @error('code')
                        <div class="alert alert-modern alert-modern-danger mt-3 mb-0">{{ $message }}</div>
                    @enderror
                </div>
            </div>

            <!-- Daftar Kelas -->
            <h1 class="h3 mb-4 gradient-heading">Daftar Kelas Anda</h1>
            <div class="row stagger-children">
                @forelse ($courses as $course)
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card card-modern h-100 shadow-sm course-card hover-lift">
