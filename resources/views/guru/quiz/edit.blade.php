        ...
        <div class="col-lg-10">
            <div class="card card-modern shadow-modern animate-fade-in">
                <div class="card-header bg-warning text-dark">
                    <h4 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Quiz</h4>
                </div>
                <div class="card-body">
                    @if($errors->any())
                    ...
                    <div class="alert alert-warning">
                        ...
                        <strong>Perhatian:</strong> Edit soal akan menghapus semua data jawaban murid yang sudah ada. Pastikan Anda yakin sebelum melakukan perubahan.
                    </div>

                    <form action="{{ route('teacher.quiz.update', [$course, $assignment]) }}" method="POST" id="quizForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="border-bottom pb-4 mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="title" required class="form-control" value="{{ old('title', $assignment->title) }}" placeholder="Contoh: Quiz Laravel Basics">
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-bold">Deskripsi</label>
                                <textarea name="description" rows="3" class="form-control" placeholder="Jelaskan tentang quiz ini...">{{ old('description', $assignment->description) }}</textarea>
                            </div>
                        ...
                        <div class="border-bottom pb-4 mb-4">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="duration_minutes" min="1" class="form-control" value="{{ old('duration_minutes', $assignment->quiz->duration_minutes) }}" placeholder="Contoh: 60">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="max_attempts" min="1" required class="form-control" value="{{ old('max_attempts', $assignment->quiz->max_attempts) }}">
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-bold">Nilai Minimum Lulus (%)</label>
                                <input type="number" name="passing_score" min="0" max="100" step="0.01" class="form-control" value="{{ old('passing_score', $assignment->quiz->passing_score) }}" placeholder="Contoh: 70">
                                <small class="text-muted">Kosongkan jika tidak ada passing score</small>
                            </div>

                            <div class="border p-3 rounded bg-light">
                                <div class="form-check mb-2">
                                    <input type="checkbox" name="show_results_immediately" value="1" class="form-check-input" id="showResults" {{ old('show_results_immediately', $assignment->quiz->show_results_immediately) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="showResults">Tampilkan hasil langsung setelah submit</label>
                                </div>

                                <div class="form-check mb-2">
                                    <input type="checkbox" name="shuffle_questions" value="1" class="form-check-input" id="shuffleQ" {{ old('shuffle_questions', $assignment->quiz->shuffle_questions) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="shuffleQ">Acak urutan soal</label>
                                </div>

                                <div class="form-check">
                                    <input type="checkbox" name="shuffle_options" value="1" class="form-check-input" id="shuffleO" {{ old('shuffle_options', $assignment->quiz->shuffle_options) ? 'checked' : '' }}>
                        ...
                        <div class="d-flex justify-content-between pt-3 border-top">
                            ...
                            <button type="submit" class="btn btn-warning" onclick="return confirm('Update quiz akan menghapus data jawaban murid. Lanjutkan?')">
...
<script>
let questionCount = 0;
const existingQuestions = @json($assignment->quiz->questions);
...
function addQuestion(existingData = null) {
    ...
    const questionText = existingData?.question_text || '';
    const questionType = existingData?.question_type || 'multiple_choice';
    const points = existingData?.points || 1;
    const explanation = existingData?.explanation || '';
    
    questionDiv.innerHTML = `
        <div class="card-header bg-warning bg-opacity-10 d-flex justify-content-between align-items-center">
            <h6 class="mb-0 fw-bold text-warning">Soal #${questionCount + 1}</h6>
        ...
        <div class="card-body">
            <div class="mb-3">
                ...
                <textarea name="questions[${questionCount}][question_text]" required rows="2" class="form-control" placeholder="Tulis pertanyaan di sini...">${questionText}</textarea>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" name="questions[${questionCount}][points]" min="0" step="0.01" required value="${points}" class="form-control">
                </div>
            </div>

            <div id="options-container-${questionCount}" style="display: ${questionType === 'essay' ? 'none' : 'block'}">
            ...
            <div class="mt-3">
                ...
                <textarea name="questions[${questionCount}][explanation]" rows="2" class="form-control" placeholder="Penjelasan untuk jawaban yang benar...">${explanation}</textarea>
            </div>
        </div>
    `;
    ...
    if (questionType === 'multiple_choice') {
        if (existingData?.options) {
            existingData.options.forEach((option, index) => {
                addOptionWithData(questionCount, index, option.option_text, option.is_correct);
            });
        } else {
